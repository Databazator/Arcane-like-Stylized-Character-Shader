Shader "ArcanelikeStylized/StylizedCharacterAreaRimLightShader"
{
    Properties
    {
        [Header(Debug)] [Space]
        ...
        _RimLightAreaWidth("Rim Light Width", Range(0.0, 50)) = 0.25
        _RimLightAreaHeight("Rim Light Height", Range(0.0, 50)) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            HLSLPROGRAM
                ...
                float _RimLightAreaWidth;
                float _RimLightAreaHeight;
                ...
                float3 OffsetPositionInBasis(float3 position, float3 offset, float3 basisForward, float3 basisUp, float3 basisRight)
                {
                    float3 offsetRebased = float3(
                        offset.x * basisRight +
                        offset.y * basisUp +
                        offset.z * basisForward
                        );

                     return position + offsetRebased;
                }
                ...
                {      
                    ...
                    float rimLightingMask = 0;
                    float3 rimLightPosition = 0;
                    float3 toRimLightDirection = 0;

                    if(abs(_RimLightAreaWidth) >= 0.0001f)
                    {
                        for(float x = -abs(_RimLightAreaWidth)/2.0; x <= abs(_RimLightAreaWidth)/1.5; x+= abs(_RimLightAreaWidth))
                        {
                            float3 offset = _RimLightOffset + float3(x, 0, 0);
                            rimLightPosition = OffsetPositionInBasis(i.positionWS.xyz, offset, forward, up, side);
                            toRimLightDirection = normalize(rimLightPosition - i.positionWS) * -1;
                            // add current sample's lighting value and clamp to 0-1
                            rimLightingMask = saturate( rimLightingMask + saturate(dot(normalWS, toRimLightDirection))); 
                        }
                    }
                    if(abs(_RimLightAreaHeight) >= 0.0001f)
                    {
                        for(float y = -abs(_RimLightAreaHeight)/2.0; y <= abs(_RimLightAreaHeight)/1.5; y+= abs(_RimLightAreaHeight))
                        {
                            float3 offset = _RimLightOffset + float3(0, y, 0);
                            rimLightPosition = OffsetPositionInBasis(i.positionWS.xyz, offset, forward, up, side);
                            toRimLightDirection = normalize(rimLightPosition - i.positionWS) * -1;
                            // add current sample's lighting value and clamp to 0-1
                            rimLightingMask = saturate( rimLightingMask + saturate(dot(normalWS, toRimLightDirection))); 
                    ...
                    // sample the rim light "area" centre as well
                    rimLightPosition = OffsetPositionInBasis(i.positionWS.xyz, _RimLightOffset, forward, up, side);
                    toRimLightDirection = normalize(rimLightPosition - i.positionWS) * -1;
                    rimLightingMask =  saturate( rimLightingMask + saturate(dot(normalWS, toRimLightDirection)));

                    
                    // Apply the rim lighting mask                    
                    rimLightingMask *= rimLightingPassMask;
                    // ------------------------------------------------------
                                        

                    //Calcualted Rim Lighting step
